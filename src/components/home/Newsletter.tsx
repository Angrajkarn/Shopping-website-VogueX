...
const handleSubscribe = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
        const { api } = await import("@/lib/api") // Dynamic import to avoid cycles if any
        const res = await api.subscribeNewsletter(email)
        alert(res.message || "Subscribed!") // Simple alert for now, could be toast
        setEmail("")
    } catch (error) {
    ...
    <section className="relative py-24 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
            ...
            <img
                ...
                style={{ transform: 'translateY(-10%)' }} // Slight parallax hint
            />
        </div>
